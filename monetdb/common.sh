...
# Check if a monetdb server is running
if [ -f $DATADIR/merovingian.pid ]
then
  echo "A monetdb server is already running: pid file present. Exiting."
  exit -2
fi

# Check if a monetdb config file is present, sets user and pw to avoid prompt
if [ ! -f $HOME/.monetdb ]
then
  mv $CREDENTIALSFILE $HOME/.monetdb
fi
